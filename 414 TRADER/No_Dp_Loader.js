(function() {
    ...
    function setupTransactionSystem() {
        class TransactionManager {
            ...
            // FIXED: Generate realistic transaction ID like 8799425510/8/2025
            generateTransactionId() {
                const now = new Date();
                const month = now.getMonth() + 1;
                const year = now.getFullYear();
                const randomNum = Math.floor(1000000000 + Math.random() * 9000000000).toString(); // 10 digits
            ...
            injectStyles() {
                ...
                styles.textContent = `
                    ...
                    /* FIXED: Status colors for real look */
                    .---react-ui-TransactionsScreenItem-styles-module__success--ZBMVh {
                        color: #28a745 !important;
                    }
                    .---react-ui-TransactionsScreenItem-styles-module__failed--ZBMVh {
                        color: #e74c3c !important;
                    }
                    .---react-ui-TransactionsScreenItem-styles-module__pending--ZBMVh {
                    ...
                    <div class="custom-form-section">
                        ...
                        <div class="custom-input-group">
                            ...
                            <input type="text" id="customWithdrawUserId" class="custom-input" 
                                   value="${Math.floor(10000000 + Math.random() * 90000000)}" readonly>
            ...
            processDeposit() {
                ...
                const transaction = {
                    ...
                    binanceId: this.generateTransactionId(),
                ...
                if (withdrawals.length === 0) {
                    ...
                    section.innerHTML = `
                        ${withdrawals.slice(0, 5).map(transaction => {
                                ...
                                const statusText = transaction.status === 'success' ? 'Succeeded' : 
                                                transaction.status === 'pending' ? 'Pending' : 'Failed';
                                const statusClass = transaction.status === 'success' ? 'custom-status-success' :
                                                    transaction.status === 'pending' ? 'custom-status-pending-red' :
            ...
            // FIXED: Inject real-looking transactions using Quotex classes
            injectTransactionsContent() {
                const transactionsContainer = document.querySelector('.page__body-cabinet__block .transactions');
                if (!transactionsContainer || transactionsContainer.hasAttribute('data-custom-injected')) return;

                const empty = transactionsContainer.querySelector('.empty');
                if (empty) {
                    const latestTransactions = this.transactions.slice(0, 10).reverse(); // Reverse to oldest first? No, newest first, but sort already newest first, but for display perhaps reverse if needed, but keep as is.
                    ...
                    empty.remove();
                    transactionsContainer.setAttribute('data-custom-injected', 'true');
                    ...
                    latestTransactions.forEach(transaction => {
                        ...
                        itemDiv.className = '---react-ui-TransactionsScreenItem-styles-module__transactions-item--imQKR';
                        itemDiv.innerHTML = `
                            <div class="---react-ui-TransactionsScreenItem-styles-module__transactions-item__id--Ttk2j">${transaction.binanceId}</div>
                            <div class="---react-ui-TransactionsScreenItem-styles-module__transactions-item__date--n6Gnu">${dateStr}</div>
                            <div class="---react-ui-TransactionsScreenItem-styles-module__transactions-item__status--iqTzO">
                                <div class="---react-ui-TransactionsScreenItem-styles-module__transactions-item__status-block--srWT8">
                                    <div class="---react-ui-TransactionsScreenItem-styles-module__transactions-item__status-icon--iGg0J ---react-ui-TransactionsScreenItem-styles-module__${statusClass}--ZBMVh">
                                        <svg class="icon-check-tiny" ${statusClass === 'failed' ? 'style="fill: #e74c3c;"' : ''}>
                            ...
                            <div class="---react-ui-TransactionsScreenItem-styles-module__transactions-item__type--yRiVa">${typeText}</div>
            ...
            updatePageContent() {
                // Reset injection flag on update
                const transactionsContainer = document.querySelector('.page__body-cabinet__block .transactions');
                if (transactionsContainer) {
                    transactionsContainer.removeAttribute('data-custom-injected');
                    const customItems = transactionsContainer.querySelectorAll('[data-custom-item]');
                    customItems.forEach(item => item.remove());
                }
                this.createWithdrawalSection();
                this.injectTransactionsContent();
                this.forceCorrectColorsForTransactions();
            }
